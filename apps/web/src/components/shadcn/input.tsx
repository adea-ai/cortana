...
// handlers through input so typing, pasting, and file selection update live.
...
function invokeInputHandler(handler: unknown, event: Event) {
...
function Input(props: ComponentProps<'input'>) {
  const [local, rest] = splitProps(props, ['class', 'type', 'onChange', 'onInput'])
  return (
    <input
      type={local.type}
      onInput={(event) => {
        invokeInputHandler(local.onInput, event)
        // File pickers fire both input and change natively; relaying would
        // invoke the handler twice per selection.
        if (local.type !== 'file') invokeInputHandler(local.onChange, event)
      }}

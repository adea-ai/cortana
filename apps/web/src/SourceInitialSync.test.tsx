...
test('settings bridge failures expose a retry action', async () => {
  ...
  const loaded = {
    value: null as DesktopSettings | null,
  }
  render(() => (
    <SettingsView
      onSaved={() => {}}
...
test('standalone source polling pauses while Settings is backgrounded', async () => {
  ...
  try {
    ...
    await waitFor(() => expect(state.pollCount).toBeGreaterThan(0), {
      timeout: 1_500,
    })
  } finally {
    window.confirm = originalConfirm
...
test('initial sync plans a fixed budget and displays the native limits', async () => {
  ...
  try {
    ...
    expect(state.planCalls.at(-1)).toEqual({
      source: 'work-code',
      budget: 'medium',
    })
  } finally {
...
test('initial sync flow disappears safely when its source is reloaded away', async () => {
  ...
  render(() => <SettingsView onSaved={() => {}} initialSection="sources" desktopSettings={ds()} />)
  await waitFor(() =>
    expect(
      screen.getByRole('button', {
        name: 'Initial sync',
...
test('a plan without validation coverage gates the start behind budget validation', async () => {
  ...
  try {
    state.planOverrides = {
      validation_covers_budget: false,
    }
    openSources()
    await waitFor(() =>
      expect(
    ...
    fireEvent.click(
      screen.getByRole('button', {
        name: 'Validate for this budget',
      })
    )
    await waitFor(() => expect(state.validationCalls).toHaveLength(1))
    expect(state.validationCalls[0]).toEqual({
      source: 'work-code',
      budget: 'small',
    })
  } finally {
...
test('shared source-job snapshots unlock the initial-sync plan without local polling', async () => {
  ...
  try {
    ...
    render(() => <SettingsView onSaved={() => {}} initialSection="sources" sourceJobs={jobs()} />)
    await waitFor(() =>
      expect(
        screen.getByRole('button', {
          name: 'Initial sync',
...
test('evicting a shared source job clears its stale local snapshot', async () => {
  ...
  const [jobs, setJobs] = createSignal<DesktopSourceJob[]>([activeJob])
  render(() => <SettingsView onSaved={() => {}} initialSection="sources" sourceJobs={jobs()} />)
  await waitFor(() => expect(screen.getByText('Trial sync · running')).toBeTruthy())
  setJobs([])

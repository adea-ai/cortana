...
for (const id of jobs) {
  test(`${id} isolates its Rust cache by workload, platform and toolchain`, () => {
    ...
    const prefix = '${{ runner.os }}-${{ runner.arch }}-desktop-rust-v2-${{ github.job }}-'
...
test("parallel metadata jobs cannot reserve another job's immutable cache key", () => {
  const resolved = jobs.map((id) => key(id).replaceAll('${{ github.job }}', id))
  assert.equal(new Set(resolved).size, jobs.length)
})
...
test('metadata and dependency audit jobs never transfer compilation targets', () => {
  for (const id of ['gtk_provenance', 'security_audit']) {
    assert.deepEqual(paths(id), ['~/.cargo/registry', '~/.cargo/git'])
  }
})
...
test('all six checks stay independent and the aggregate still gates every one', () => {
  ...
  assert.match(job('release'), /CORTANA_DESKTOP_TARGET: x86_64-unknown-linux-gnu/)
